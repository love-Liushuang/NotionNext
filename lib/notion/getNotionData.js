...
export function getNavPages({ allPages }) {
  ...
  return allNavPages.map(item => ({ id: item.id, title: item.title || '', image: item.image || '', pageCoverThumbnail: item.pageCoverThumbnail || '', category: item.category || null, tags: item.tags || null, summary: item.summary || null, slug: item.slug, pageIcon: item.pageIcon || '', lastEditedDate: item.lastEditedDate }))
}
...
async function getDataBaseInfoByNotionAPI({ pageId, from }) {
  ...
  console.log('allNavPages-----', allNavPages);
